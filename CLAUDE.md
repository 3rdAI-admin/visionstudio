...
| CORS errors from browser                                                                                                                                                      | Backend missing `cors` middleware                                                                            | `app.use(cors({ origin: process.env.FRONTEND_ORIGIN || 'http://localhost:3000' }))` — `FRONTEND_ORIGIN` is set automatically by `startup.sh`/the Settings UI when ports are non-default                                                                                                   |
