...
Last updated: 2026-09-03, by session `01G4jeWmRThqhH8xZKFyuf1T`.
...
- **iOS app**: Capacitor-wrapped, Xcode project at `ios/App/App.xcodeproj`, bundle ID `com.th3rdai.visionstudio`. **Built and verified working on iPhone 17 Simulator** (installed, launched, confirmed a real network round-trip to the LAN backend — settings icon's status dot went blue, meaning `/api/key-status` succeeded end-to-end through CORS + ATS).
  - Built with `VITE_BACKEND_URL=http://192.168.50.7:3001 npm run build && npx cap sync ios` — that IP is this Mac's current LAN address (`ipconfig getifaddr en0`), will change if the network changes.
...
- **Real device testing unconfirmed** — the user's iPhone shows as "Offline" in `xcrun xctrace list devices` (not connected/trusted right now). Only the Simulator has been verified. To test on the real device: connect via cable or ensure prior wireless-debugging pairing, trust the Mac if prompted, then `xcodebuild ... -destination "platform=iOS,id=<device-id>"`.
- **No hosted/production backend exists.** `HOSTED=true` and `VITE_BACKEND_URL` are wired up in code but nothing is deployed — the LAN-IP build only works on the same Wi-Fi as this Mac. A real device off that network, or a TestFlight/App Store build, needs a real HTTPS backend.
