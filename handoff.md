...
Last updated: 2026-09-03 (Archon project created; doc-drift item closed), by session `01PbFrBceHHpLS3N8FhU8pqQ`.

## Current state (verified, not assumed)

- **Repo**: `/Users/james/Projects/visionstudio` (renamed from `visionedit` in `b21b85a`, 2026-09-02).
- **HEAD**: `59afd0f` — "Document iOS/Capacitor build steps and HOSTED=true production mode". Working tree clean except this file.
...
- **Open PR #5** (dependabot, `dependabot/npm_and_yarn/npm_and_yarn-b1effbdb68`): bumps 11 packages across root + `backend/`, including **Vite 6.4.2 → 8.2.2** (major version jump) and `sharp` 0.34.5 → 0.35.0 (drops Node <20.9 support — this Mac runs Node v24.8.0, fine). **Checked out in an isolated worktree and verified 2026-09-03**: `npm install` clean in both root and `backend/`, `tsc --noEmit` clean, `npm run build` succeeds (only a forward-compat warning — `vite.config.ts`'s `__dirname` usage will need `import.meta.dirname` in a future Vite major, not this one), `npm run test:run` 35/35 passing, matching the pre-PR baseline. **e2e run 2026-09-03 in a follow-up pass** (`playwright.config.ts` reuses an existing server unless `CI` is set, which would've silently tested the live main-branch dev server instead of the PR branch — avoided by briefly `./startup.sh stop`-ping the real dev servers, running the PR worktree's own backend + `CI=1 npm run e2e` on 3001/3002, then `./startup.sh start` to restore the user's session): **4/4 e2e tests pass**, matching baseline. Build+typecheck+unit+e2e all green on PR #5 — no breakage found from the Vite 6→8 jump. Only touches `package.json`/`package-lock.json`, no source or config changes needed on our side. **Clear to merge.**
